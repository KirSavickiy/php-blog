...
$endpage = ceil($count['total'] / 10);
$pages = range(1, $endpage);


if (isset($_GET['page']) && ($_GET['page'] >= 2) && ($_GET['page'] <= end($pages))) {
    $offset = (int) ($_GET['page'] * 10 - 10);
    $sql = "SELECT * FROM article ORDER BY createdAt DESC LIMIT 10 OFFSET :offset";
    ...
    $sql = "SELECT * FROM article ORDER BY createdAt DESC LIMIT 10;";
    $stmt = $pdo->query($sql);
    $articles = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
